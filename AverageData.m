...
function [parameters] = AverageData(parameters)
    ...
    if isfield(parameters, 'useSqueeze') && ~parameters.useSqueeze
        %  Take the mean
        average = mean(parameters.data, parameters.averageDim, 'omitnan'); 
    
        % Take the standard deviation
        std_dev = std(parameters.data, 0, parameters.averageDim, 'omitnan'); 
    else
         %  Take the mean
        average = squeeze(mean(parameters.data, parameters.averageDim, 'omitnan')); 
    
        % Take the standard deviation
        std_dev = squeeze(std(parameters.data, 0, parameters.averageDim, 'omitnan')); 
    end
