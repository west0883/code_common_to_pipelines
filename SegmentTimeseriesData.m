        ...
        for dayi=1:size(mice_all(mousei).days, 2)
            ...
            for stacki= 1:size(stackList.filenames,1)
                ...
                for periodi = 1:size(periods_all,1)
                    ...
                    % Do the next part only if user said to use a strict 
                    % time window AND they want to use full offsets
                  
                    if isfield(parameters, 'use_set_window') && parameters.use_set_window && isfield(parameters, 'full_transition_flag') && parameters.full_transition_flag
                    
                        % Change the time window to look at if it's a full
                        % transition.
                        if strcmp (period, 'full_onset') | strcmp(period, 'full_offset')
                            time_window_use = (parameters.time_window_seconds + parameters.full_transition_extra_time)*2;
                        else
                            time_window_use = parameters.time_window_seconds;
                        end 
                    end
                    ...
                    if isempty(segment_ranges)
                        ...
                        for instancei = 1 : size(segment_ranges, 1)
                            ...
                            % If user says so, get only the number of points 
